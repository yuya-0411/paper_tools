...
章立て，博士論文全体の中心研究課題，各章の論証契約もプロジェクト本体に含まれます。ログインだけでは送信されず，利用者が対象プロジェクトの「クラウドへコピー」または更新操作を明示的に実行した場合だけ保存されます。論証契約は法的契約や検証済み証拠ではなく，執筆上の整理メモです。
...
アプリ更新後は，既存のSupabaseプロジェクトでも最新の`supabase/schema.sql`を再実行してください。現在の`save_project`は版番号だけでなくpayloadの`schemaVersion`も同一の更新条件で比較します。`schemaVersion`がない既存payloadはv1として扱い，新しいpayloadの形式が保存済みpayloadより古い場合は上書きせず`conflict`になります。ブラウザーキャッシュ等に残った古い公開ページから，schema v2の章立てや論証契約が消されることを防ぐためです。
...
| `conflict` | 別端末が先に保存済み，または保存済みpayloadより古い`schemaVersion` | 上書きせず、公開ページを更新してクラウド最新版を読み直す |
| `not_found` | 対象がない、または削除済み | 自動再作成せず利用者へ確認する |

RPCは「確認した版番号と現在の版番号が同じで，新しいpayloadの`schemaVersion`が現在以上の場合だけ更新」をデータベース内で一度に行います。明示されたschemaVersionは正の整数だけを許可し，不正な既存値にも安全側で停止します。競合時に原稿本文を応答へ含めないため、他人のデータの有無や内容は返りません。
...
- schema v2を保存した後，古いschemaVersionのpayloadでは同じ版番号でも上書きできない
